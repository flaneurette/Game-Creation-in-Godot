...
@export var anim_name: StringName = &"mixamo_com"
...
## Leave empty to freeze the last pose instead.
@export var anim_idle_name: StringName = &""
...
func _update_walk_animation(delta: float) -> void:
	...
	if anim_speed > anim_move_threshold:
		# play() only on an actual transition. Calling it blindly every frame,
		# or pairing it with stop(), rewinds the clip to frame 0 and makes the
		# skeleton pop.
		if anim_player.current_animation != anim_name or not anim_player.is_playing():
			anim_player.play(anim_name)
		anim_player.speed_scale = clampf(
			anim_speed / maxf(anim_walk_speed_reference, 0.001),
			anim_speed_scale_min,
			anim_speed_scale_max
		)
	else:
		if anim_idle_name != &"":
			...
			# pause() holds the current pose; stop() would rewind to frame 0
			# and pop the skeleton.
			anim_player.pause()
